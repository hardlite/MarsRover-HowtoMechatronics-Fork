...
#define motorW1_IN1 6 //left front
#define motorW1_IN2 7
#define motorW2_IN1 4 //left middle
#define motorW2_IN2 5
#define motorW3_IN1 2 //left back
#define motorW3_IN2 3
#define motorW4_IN1 13 //right front
#define motorW4_IN2 10
#define motorW5_IN1 8 //right middle
#define motorW5_IN2 9
#define motorW6_IN1 11 //right back
#define motorW6_IN2 12

#define CH_STEERING 0
#define CH_CAM_TILT 1
#define CH_CAM_PAN 3
#define CH_THROTTLE 2
#define CH_FORWARD_REVERSE 6
...
int valSteering, valCamTilt, valThrottle, valCamPan, valForwardReverse = 0;
int servo1Angle = 90;
int servo3Angle = 90;
int servo4Angle = 90;
int servo6Angle = 90;
...
void loop() {
  ...
  IBus.loop();
  valSteering = IBus.readChannel(CH_STEERING);
  valCamTilt = IBus.readChannel(CH_CAM_TILT);
  valThrottle = IBus.readChannel(CH_THROTTLE);
  valCamPan = IBus.readChannel(CH_CAM_PAN);
  valForwardReverse = IBus.readChannel(CH_FORWARD_REVERSE);
  ...
  Serial.print(valCamPan);
  Serial.print(" | Forward/Reverse = ");
  Serial.println(valForwardReverse);

  // Converting the incoming data
  // Steering right
  if (valSteering > 1515) {
    r = map(valSteering, 1515, 2000, 1400, 600); // turining radius from 1400mm to 600mm
  }
  // Steering left
  else if (valSteering < 1485) {
    r = map(valSteering, 1485, 1000, 1400, 600); // turining radius from 600mm to 1400mm
  }
  // Rover speed in % from 0 to 100
  s = map(valThrottle, 1000, 2000, 0, 100); // rover speed from 0% to 100%
  ...
  if (valCamPan >= 1000 && valCamPan < 1300) {
    camPan = map(valCamPan, 1000, 1485, 400, 0);
  }
  else if (valCamPan > 1700 && valCamPan <= 2000) {
    camPan = map(valCamPan, 1515, 2000, 0, -400);
  }
  else {
    camPan = 0;
  }
  ...
  if (valSteering > 1515) {
    ...
    // DC Motors
    if (valForwardReverse < 1500) { // Move forward
      // Motor Wheel 1 - Left Front
    ...
    else if (valForwardReverse > 1500) {
      // Motor Wheel 1 - Left Front
  ...
  // Steer left
    ...
    else if (valForwardReverse > 1500) { // Move backward
      // Motor Wheel 1 - Left Front
      digitalWrite(motorW1_IN1, LOW);   // PWM value
...
void calculateMotorsSpeed() {
  ...
  if (valSteering > 1485 && valSteering < 1515) {
  ...
  // when steering, wheels speed depend on the turning radius value
    ...
    // Due to the rover geometry, all three outer wheels should rotate almost with the same speed. They differ only 1% so we asume they are the same.
    speed1 = s;
